...
**Multiplayer:** deploy (below) and share the URL. Everyone in the same *colony* (world) terraforms the same planet. The menu lists active colonies to join — or type a new name to found one (`?world=name` in the URL works too). Founding a colony with a **password** locks it: only players with the password can join (🔒 in the colony list), while open colonies stay open forever. The password is remembered per-world in your browser, so the founder never retypes it; only a salted-free SHA-256 hash is stored server-side. A password can only be set at creation.

**Colony settings (⚙ on the menu):** whoever founds a colony holds its **admin key** (minted at creation, kept in the founder's browser; on colonies older than this feature, the first joiner after the update inherits it). The founder can retune **world rules for everyone** — plant growth ½–4×, mining speed ½–4×, suit O₂ drain ½–2×, and whether creative mode is open to everyone or founder-only — plus change/remove the password, **kick** players, and **wipe the world** back to bare seed. In single-player the same panel tunes your own world's rules. Other colonists appear as suited avatars with name tags and a colored **locator beam** visible across the terrain — players in creative mode glow **magenta** (◆ on their name tag) and their switch is announced in chat, so no one terraforms in god-mode unnoticed; the top-left readout lists everyone's distance and bearing, and `T` opens chat.
...
- **Self-spreading**: once the planet holds **≥5°C and ≥8% O₂**, moss, shrubs, tree canopies and the greens themselves seed **wild greens** — a pale ground cover that greens the land and adds a little O₂ (0.0015%/block) but **never grows taller. Forests are always planted**: only hand-planted lichen climbs the lichen → moss → shrub → tree ladder. Spreading is self-limiting (seeds refuse ground with 4+ plants within 2 blocks; max 2 seeds/second planet-wide), and mining wild greens yields lichen — harvest the wilds to plant real forests.
